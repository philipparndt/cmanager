...
const (
	...
	attrBlink
	attrWrap
	// AttrFaint (SGR 2, dim) — added in this local fork so cmanager can mirror
	// claude's dimmed/gray text (suggestions, hints) instead of rendering it at
	// full intensity. Exported so the renderer can read it off the glyph Mode.
	AttrFaint
)
...
func (t *State) setAttr(attr []int) {
	...
	for i := 0; i < len(attr); i++ {
		...
		case 21, 22: // bold off / normal intensity — also clears faint
